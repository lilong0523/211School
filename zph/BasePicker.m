...
#import "BasePicker.h"
@interface BasePicker()<UIPickerViewDataSource, UIPickerViewDelegate>
...
- (instancetype)initWithDic:(NSMutableArray *)arry copNum:(NSInteger)compontNum
{
    ...
    if (self) {
        ...
        currentNum= 0;
        _datasourceArry = [[NSMutableArray alloc] initWithArray:arry];
...
- (void)show
{
    ...
    if (_compontNum == 1) {
        ...
        [_sexPicker selectRow:_defaultNum inComponent:0 animated:YES];
        selectText = [[_datasourceArry objectAtIndex:_defaultNum] objectForKey:@"name"];
        currentFullId = [[_datasourceArry objectAtIndex:_defaultNum] objectForKey:@"id"];
        [titlePick setText:selectText];
    }
    else if (_compontNum == 2){
        if (self.updateBlock) {
            self.updateBlock([[_datasourceArry objectAtIndex:currentNum] objectForKey:@"id"]);
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component
{
    if (component == 0) {
        ...
        if (self.updateBlock) {
            currentNum = row;
            if ([[_datasourceArry objectAtIndex:currentNum] objectForKey:@"city"] == nil) {
                self.updateBlock([[_datasourceArry objectAtIndex:row] objectForKey:@"id"]);
...
- (void)reloadData
{
    if (_compontNum == 1) {
        ...
        currentFullId = [NSString stringWithFormat:@"%@",[[_datasourceArry objectAtIndex:index0] objectForKey:@"id"]];
    }
    else if (_compontNum == 2){
        NSInteger index0 = [_sexPicker selectedRowInComponent:0];
...
- (void)setSecondArry:(NSMutableArray *)secondArry{
    _secondArry = secondArry;
    
    NSMutableDictionary *dic = [[NSMutableDictionary alloc] initWithDictionary:[_datasourceArry objectAtIndex:currentNum]];
    [dic setObject:_secondArry forKey:@"city"];
    [_datasourceArry replaceObjectAtIndex:currentNum withObject:dic];
