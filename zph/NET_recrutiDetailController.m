...
@interface NET_recrutiDetailController ()<UITableViewDataSource,UITableViewDelegate,NET_recrutiDetaildelegate>

@end

@implementation NET_recrutiDetailController
{
...
- (void)addHeadView{
    ...
    mainTable = [[UITableView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.topView.frame), self.view.frame.size.width, self.view.frame.size.height-CGRectGetMaxY(self.topView.frame)-65) ];
 ...
 */
- (void)addBottomView{
    UIView *bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(mainTable.frame), self.view.frame.size.width, self.view.frame.size.height-CGRectGetMaxY(mainTable.frame))];
    [bottomView setBackgroundColor:[UIColor whiteColor]];
    [self.view addSubview:bottomView];
    
    BaseButton *goToMeetiing = [[BaseButton alloc] initWithFrame:CGRectMake(20, 10, bottomView.frame.size.width-40, 45)];
    [goToMeetiing addTarget:self action:@selector(GoToConference) forControlEvents:UIControlEventTouchUpInside];
    goToMeetiing.text = @"进入会场";
    [bottomView addSubview:goToMeetiing];
 ...
 */
- (void)GoToConference{
    ConferenceController *conference = [[ConferenceController alloc] init];
    conference.recrutId = _recrutId;
    [self.navigationController pushViewController:conference animated:YES];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        cell.detailText = [[datasource objectForKey:@"invitation"] isEqualToString:@""]?@"暂无信息！":[datasource objectForKey:@"invitation"];
        return cell;
    }
    else{
        static NSString *ID =@"cellID22";
        NET_recrutiDetail2Cell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
        if (cell == nil) {
            cell = [[NET_recrutiDetail2Cell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:ID ];
            
        }
        cell.backgroundColor = [UIColor clearColor];
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        
        cell.detailText = datasource;
