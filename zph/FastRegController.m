...
@interface FastRegController ()<UITextFieldDelegate>

@end

@implementation FastRegController
{
...
- (void)getVerify{
    ...
    [request postAsynRequestBody:dicBody interfaceName:GETverifyCode interfaceTag:1 parType:0];
}
...
- (void)getToken{
    ...
    request.httpSuccessBlock = ^(NSDictionary * dicData,NSInteger tag){
        ...
        if ([[dicData objectForKey:@"code"] integerValue] == 0) {
            ...
            NSMutableDictionary *passDic = [[NSMutableDictionary alloc] initWithObjectsAndKeys:[[dicData objectForKey:@"data"] objectForKey:@"mobilePhone"],@"mobilePhone",[[dicData objectForKey:@"data"] objectForKey:@"verifyCode"],@"verifyCode",[[dicData objectForKey:@"data"] objectForKey:@"token"],@"token",passWord.text,@"userPass", nil];
            basicInfoController *basicEdit = [[basicInfoController alloc] init];
            basicEdit.parDic = passDic;
            basicEdit.topTitle = @"基本信息";
            [self.navigationController pushViewController:basicEdit animated:YES];
        }
        else{
            [HUDProgress showHUD:[NSString stringWithFormat:@"%@",[dicData objectForKey:@"message"]]];
    ...
    NSDictionary *dicBody = @{
                              ...
                              @"mobilePhone":phone.text,//手机号
                              @"verifyCode":currentVerify,//验证码
                              };//json data
    
    
    [request postAsynRequestBody:dicBody interfaceName:GETTOKEN interfaceTag:1 parType:0];
}
